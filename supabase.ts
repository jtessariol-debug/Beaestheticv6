...
// --- Constantes de Almacenamiento e Imágenes ---
...
// --- Lógica de Procesamiento de Imágenes (Canvas) ---
...
export const optimizeImageForUpload = async (
    ...
    if (file.size > maxOriginalBytes) {
        throw new Error(`La imagen es demasiado grande. Máximo: ${Math.round(maxOriginalBytes / ONE_MB)}MB.`);
    }
...
export const mapProductToRealtimeService = (product: Partial<Product>): RealtimeService => {
    ...
    // Hash simple para el ID
    const hashedId = Math.abs(sourceId.split('').reduce((a, b) => { a = ((a << 5) - a) + b.charCodeAt(0); return a & a; }, 0)) || 1;
